const card = document.getElementsByClassName("fburger");
const btnAdd = document.getElementsByClassName("finfo");
const btnCart = document.querySelector(".fsepetbutton");
const cartList = document.querySelector(".fsepetmenu");
...
class UI {
  ...
  cartCount() {
    ...
    let itemCount = document.getElementById("fsepetsayı");
...
for (let i = 0; i < card.length; i++) {
  btnAdd[i].addEventListener("click", function (e) {
    let title = card[i].getElementsByClassName("ftitle")[0].textContent;
    let price = card[i].getElementsByClassName("ffiyat")[0].textContent;
    let image = card[i].getElementsByClassName("fburgerimg")[0].src;
